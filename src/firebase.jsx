...
export async function storeUserdata(
  ...
  if (teacherStatus === "yes") {
    ...
    if (exist) {
      console.log(
        "I am a teacher and my college already exist, that means I am a 2nd or nth teacher"
      );
    } else {
...
export async function createComplain(CollegeName, CollegeId, obj) {
  ...
  console.log(path);
 let array=  await getEmail(path);
 //This array contains the email of teachers we have to call the function here to send the email teacher
...
// Creating the email part
 async function getEmail(path){
const dbref =  ref(getDatabase());
let arr = [];
let snapshot = await get(child(dbref,path));
console.log("In the get email")
snapshot.forEach((childSnapshot)=>{
  arr.push(childSnapshot.val().Email)
